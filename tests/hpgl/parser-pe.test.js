...
describe('parseHpgl PE commands', () => {
  it('creates millimetre polylines and line boundaries around pen-up pairs', () => {
    ...
    expect(result.geometries).toEqual([
      {
        type: 'polyline', layer: 'a', fileName: 'a.hpgl', offset: 0,
        points: [[0, 0], [1, 0], [2, 1]],
      },
      {
        type: 'line', layer: 'a', fileName: 'a.hpgl', offset: 0,
        points: [[3, 1], [3, 2]],
      },
    ]);
  ...
  it('keeps an open polyline and position unchanged when PE decoding fails', () => {
    const malformed = [
      flag(':'), ...encodeValue(3),
      ...encodeValue(40), ...encodeValue(0),
      63,
    ...
    expect(result.geometries).toEqual([
      {
        type: 'polyline', layer: 'a', fileName: 'a.hpgl', offset: 4,
        points: [[0, 0], [1, 0], [2, 0]],
      },
    ]);
