...
`dxf-writer`と`@tarikjabiri/dxf`は週1万件以上ダウンロードされているが、安定版の公開が2年以上止まっている。前者は公開APIが主にレイヤープロパティを扱い、後者は公式説明上AC1021以降を対象としている。今回必須のAC1015オブジェクト構造へ合わせるにはライブラリ内部への依存または改変が必要になり、汎用化よりも互換性リスクが高くなるため採用しない。
...
既存の入力検証、文字エスケープ、図形の座標・レイヤー検証を維持する。最初に標準テーブル、ブロック、オブジェクト、全図形のhandleと所有関係を割り当ててオブジェクトグラフを確定し、その後に各sectionを直列化する。この2段階処理により、先頭の`$HANDSEED`と後続参照の整合性を保証する。公開関数と戻り値の`string[]`は変更しない。
...
既存の座標、半径、角度、文字高、回転、頂点数の検証は変更しない。LAYER tableの色はACI 7とし、各entityでは色のgroup code 62を省略する。
...
- 既存のレイヤー、Unicode、座標、円弧、TEXT、入力検証テストを維持し、LAYER tableのACI 7とentity group code 62不在を検査する。

### 結合テスト

- `reference`の8ファイルからDXFを生成し、8ファイル、エラー0件、警告0件であることを検査する。
...
- 全entityを入力HPGLの期待図形と同じ順序でcanonical化し、type、layer、および型固有のgeometryまたはtextを全件比較する。期待図形数とentity数の一致、およびentity group code 62の不在も検査する。
...
- 警告、修復要求、クラッシュなしに開ける。
- 8レイヤーを単独表示できる。
- 全entityのColorがByLayerであり、レイヤーの色、線種、線幅変更へ追従する。
...
- ペン設定以外のHPGLパーサー動作と座標変換
- UIとWorkerの公開契約
- レイヤー命名規則
- HPGLの`SP`および`PE`内のペン変更を図形、診断、DXF出力へ影響させない規則
- LBのTEXT変換
- 出力ファイル名とダウンロード動作
