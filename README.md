...
3. 最初に追加できた元ファイル名からDXFの既定名が一度だけ自動設定されます。たとえば `drawings.zip` なら `drawings.dxf` になります。必要に応じて出力ファイル名を変更できます。その後ファイルを追加しても、既定名や変更した名前は自動変更されません。
4. 「DXFに変換」を押します。
5. 変換完了時に生成されたDXFの自動ダウンロードが始まります。
6. 変換結果を確認できます。自動ダウンロードを開始できなかった場合や、もう一度取得したい場合は、結果欄のボタンから再ダウンロードできます。
